...
//returns a new point clamped within the boundary.
Boundary.prototype.clamp = function(point) {
  return Vector(
    Math.min(Math.max(this.left, point.x), this.right),
    Math.min(Math.max(this.top, point.y), this.bottom)
  )
}
...
Boundary.prototype.nearestIntersect = function(point, velocity) {
  ...
  isect = (distX < distY) ? Vector(distX, point.y) : Vector(point.x, distY)
